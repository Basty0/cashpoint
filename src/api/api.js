...
const API_URL = "https://cash-point.sopera.mg/api"; // Remplacez par l'URL de votre API
...
// Fonction pour obtenir un récapitulatif général des transactions pour une date et un user donné
export const getGeneralTransactionRecap = async (userId, date) => {
  const token = await getAuthToken();
  const response = await axios.get(`${API_URL}/transactions/general-recap`, {
    headers: { Authorization: `Bearer ${token}` },
    params: { user_id: userId, date: date },
  });
  return response.data;
...
// Fonction pour obtenir un rapport des transactions effectuées par opérateur pour une date et un user donné
export const getTransactionReportByOperator = async (userId, date) => {
  const token = await getAuthToken();
...
// Fonction pour lister les transactions d'un user, type, et date donnés
export const getTransactionsByUserTypeAndDate = async (userId, type, date) => {
  const token = await getAuthToken();
  const response = await axios.get(
    `${API_URL}/transactions/by-user-type-date`,
    {
      headers: { Authorization: `Bearer ${token}` },
      params: { user_id: userId, type: type, date: date },
    }
  );
  return response.data;
};
export const getRaportTransactionOperateur = async (date) => {
  const token = await getAuthToken();
  const response = await axios.get(`${API_URL}/transactions/operator-report`, {
    headers: { Authorization: `Bearer ${token}` },
    params: { date: date },
  });
  return response.data;
...
export const getDashboardSummary = async (date) => {
  try {
    ...
    console.error(
      "Erreur lors de la récupération du résumé du tableau de bord:",
      error
    );
    throw error;
  }
...
// Récupérer les détails des transactions pour le tableau de bord
export const getTransactionDetails = async (date) => {
  try {
    const token = await getAuthToken();
    const response = await axios.get(`${API_URL}/dashboard/details`, {
      headers: { Authorization: `Bearer ${token}` },
      params: { date: date },
    });
    return response.data;
    ...
    console.error(
      "Erreur lors de la récupération des détails des transactions:",
      error
    );
    throw error;
  }
